DROP DATABASE IF EXISTS adt3_ejemplo2;

CREATE DATABASE adt3_ejemplo2;
...
//
CREATE PROCEDURE matricular3 (p_idAlumno INT, p_idAsignatura INT, OUT p_salida INT)
BEGIN
	DECLARE numeroRegistros INT;
    DECLARE numeroRegistrosAlumno INT;
	DECLARE numeroRegistrosAsignatura INT;
...
IF (numeroRegistrosAlumno = 1) THEN
BEGIN
SET p_salida = 1
END;
END IF;
IF (numeroRegistrosAsignatura = 1) THEN
SET p_salida = 2
END;
END IF;
...
IF (numeroRegistrosAlumno = 1) AND (numeroResistrosAsignatura= 1) AND (numeroRegistros = 1)
